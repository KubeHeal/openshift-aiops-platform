...
# Supports: non-interactive mode (FORCE=true)
set -euo pipefail
...
echo "Step 1/5: Deleting Pattern CR..."
if oc get pattern self-healing-platform -n openshift-operators &>/dev/null; then
  oc delete pattern self-healing-platform -n openshift-operators --timeout=60s || true
...
echo "Step 2/5: Deleting ArgoCD applications..."
...
# Step 3: Delete platform namespace
echo "Step 3/5: Deleting platform namespace (may take 2-5 minutes)..."
...
# Step 4: Delete hub namespace
echo "Step 4/5: Deleting hub namespace..."
if oc get namespace self-healing-platform-hub &>/dev/null; then
  ...
  echo "Hub namespace deleted"
else
  echo "Hub namespace not found (already deleted)"
fi
